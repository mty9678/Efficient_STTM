...
% global * using some global variables
...
Ker = zeros(n,m);
 for i = 1:n
     for j = 1:m
         for k = 1:Order
             % the vectors U and V are the vector of TT-cores
             f1 = pos(k):(pos(k+1)-1);  % taking position of each tt-core
             u{k} = reshape(U(i,f1), [R(k),dims(k),R(k+1)]); % converting each core into a tensor form
