function [Bestcv, Bestc,Bestg,time_tr,time_te]= TrainAverAcc_MMK(X,label,R,k,data_KCP,choose,c1,c2,g1,g2);
...
for log2g = g1:g2 
    for cv=1:k
        ...
        for p=1:length(b)
            for q=1:p
                if Order~=1   
                    Ktrain(p,q)=Ker_DuSK(data_KCP{b(p),1},data_KCP{b(q),1},Order,2^log2g,R,flagker);
                else
                    Ktrain(p,q)=Ker_DuSK(X(b(p),:),X(b(q),:),Order,2^log2g);
                end
                if p~=q
        ...
        for r=1:n-length(b)
            for p=1:length(b)
                if Order~=1
                    Ktest(r,p)=Ker_DuSK(data_KCP{choose{cv,1}(1,r),1},data_KCP{b(p),1},Order,2^log2g,R,flagker);
                else
                    Ktest(r,p)=Ker_DuSK(X(choose{cv,1}(1,r),:),X(b(p),:),Order,2^log2g);
                end
            end
        end
        time_te1=toc;
