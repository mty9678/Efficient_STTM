function [Bestcv, Bestc,Bestg,time_tr,time_te]= TrainAverAcc_DuSK(X,label,R,k,X_CP,choose,c1,c2,g1,g2);
...
for log2g = g1:g2 
    for cv=1:k
        ...
        for p=1:length(b)
            for q=1:p
                if Order~=1   
                    Ktrain(p,q)=Ker_DuSK(X_CP{b(p),1},X_CP{b(q),1},Order,2^log2g,R,flagker);
                else
        ...
        for r=1:n-length(b)
            for p=1:length(b)
                if Order~=1
                    Ktest(r,p)=Ker_DuSK(X_CP{choose{cv,1}(1,r),1},X_CP{b(p),1},Order,2^log2g,R,flagker);
                else
