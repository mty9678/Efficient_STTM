function [Bestcv,Besttimetr,Besttimete] = SVMmain(X,label,t);
...
for i=1:t                                                                        % Repeat t times
    ...
    [~, bestc,bestg]= TrainSVM(Y,Y_label,k,Div,c1,c2,g1,g2);  % Train and select the optimal paremeters 
    for j=1:5                                                                    % Extra repeat 5 times to get more stable result
        randseed = round(rand(1)*5489);
        DivOpti=Divide(label,k,randseed);
        [cv, ~,~,timetr,timete]=TrainSVM(X,label,k,DivOpti,bestc,bestc,bestg,bestg);  
        acc=acc+cv;
