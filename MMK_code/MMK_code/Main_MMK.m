function [Bestcv,Besttimetr,Besttimete] = Main_MMK(X,label,R,data_KCP,t);
...
for i=1:t                                                                        % Repeat t times
    ...
    [~, bestc,bestg]= TrainAverAcc_MMK(Y,Y_label,R,k,data_KCP(b,1),Div,c1,c2,g1,g2);  % Train and select the optimal paremeters 
    for j=1:5                                                                    % Extra repeat 5 times to get more stable result
        randseed = round(rand(1)*5489);
        DivOpti=Divide(label,k,randseed);
        [cv, ~,~,timetr,timete]=TrainAverAcc_MMK(X,label,R,k,data_KCP,DivOpti,bestc,bestc,bestg,bestg);  
        acc=acc+cv;
